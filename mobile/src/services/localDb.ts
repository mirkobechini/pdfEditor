...
export async function savePdfLocally(pdf: LocalPdf): Promise<void> {
  ...
  await database.runAsync(
    `INSERT OR REPLACE INTO pdfs (id, user_id, original_filename, file_size, page_count, title, author, uri, created_at, updated_at, cloud_synced, cloud_synced_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      pdf.id,
      pdf.user_id ?? "",
...
export async function getUnsyncedPdfs(): Promise<LocalPdf[]> {
  const database = await getDb();
  return await database.getAllAsync<LocalPdf>(
    "SELECT *, COALESCE(cloud_synced, 0) as cloud_synced FROM pdfs WHERE cloud_synced IS NULL OR cloud_synced = 0 ORDER BY updated_at DESC",
  );
}
