...
describe("PrintOptionsModal (web)", () => {
    beforeEach(() => {
        vi.clearAllMocks();
    ...
    it("renders with auto orientation, normal margin and color selected by default", () => {
        ...
        expect(screen.getByTestId("print-color-color")).toHaveAttribute("aria-pressed", "true");
    });

    it("has no printer or copies controls — the browser's native print dialog handles those", () => {
        render(<PrintOptionsModal open={true} onClose={() => { }} onConfirm={() => { }} />);
        expect(screen.queryByTestId("print-printer-select")).not.toBeInTheDocument();
        expect(screen.queryByTestId("print-copies")).not.toBeInTheDocument();
    });

    it("lets the user pick orientation, margin and color before confirming", () => {
        ...
        fireEvent.click(screen.getByTestId("print-color-grayscale"));
