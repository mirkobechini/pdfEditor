...
| **Auth unificata web/desktop via shared**            | **Due versioni divergenti**    | **Il web ora re-exporta dal shared (`shared/src/`) la logica auth/api/tauri/error-map. Prima c'erano due copie divergenti (web senza `cloudApi`/cache). Il web ha ora il fix K5 (cache offline) e il cookie restore. `extractError` unificato a JSON raw + `mapError()`. (PR #761)**          |
