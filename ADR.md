...
| OCR con **Tesseract (pytesseract)**                 | EasyOCR, PaddleOCR         | Tesseract è leggero e standard. Richiede il binary `tesseract` sul sistema (non incluso nel sidecar desktop per ora). Se il PDF ha già testo, l'OCR non viene eseguito. Se il binary manca, l'API restituisce `503 OCR_UNAVAILABLE` con messaggio chiaro (non 500). Deploy web: installato via `apt-get` in `render.yaml`. Desktop: richiede tesseract installato a parte (issue #829, fix #831). |
...
| **Auth unificata web/desktop via shared**            | **Due versioni divergenti**    | **Il web re-exporta dal shared (`shared/src/`) tauri/error-map e i tipi. `extractError` unificato a JSON raw + `mapError()`. Il flusso auth web resta cookie-based con la copia originale `api.ts` (NON re-exporta `api.ts` dal shared: il shared ha auto-refresh 401 e CSRF diverso che rompevano login/register e2e). Il web non usa il shared `auth.tsx` (desktop-first con sidecar+cloud). (PR #761)** |
...
| **CSRF esentato per richieste Bearer-authenticated** | **CSRF sempre attivo**         | **Il middleware CSRF esenta la validazione per richieste con Bearer JWT valido e nessun cookie CSRF (mobile/desktop cloud). Il Bearer token è autenticazione esplicita (non automatica dal browser), quindi il CSRF è ridondante. Il web cookie-based resta protetto. (PR #729, issue #728)**                                                                                                              |
...
| **Import/Export per piattaforma**                    | **Solo backend online**        | **UI import/export (txt/png/jpg/gif/bmp in, txt/png/jpg/svg out) creata su tutte le piattaforme. Web e mobile usano il backend online (mobile mostra 'richiede connessione' quando offline). Desktop usa il sidecar locale (offline) con dialoghi nativi Tauri. (issue #805)**                                                                                                                             |
| **Conversione DOCX→PDF**                             | **docx-preview JS**            | **PyMuPDF non converte DOCX. Web/mobile online usano python-docx + reportlab nel backend. Desktop offline usa lo STESSO codice (python-docx + reportlab) nel sidecar PyInstaller — riuso del codice, consistenza tra piattaforme, nessuna libreria JS extra. Qualità media (testo/paragrafi). (issue #807)**                                                                                               |
| **Stampa PDF per piattaforma**                       | **Solo web**                   | **Web/Desktop usano `window.print()` su un iframe nascosto con il PDF (stampa nativa browser/webview). Mobile usa `expo-print` (`printAsync` con uri) per la stampa nativa iOS/Android (AirPrint). (issue #809)**                                                                                                                                                                                          |
