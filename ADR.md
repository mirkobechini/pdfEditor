...
| **Auth unificata web/desktop via shared**            | **Due versioni divergenti**    | **Il web re-exporta dal shared (`shared/src/`) api/tauri/error-map. `extractError` unificato a JSON raw + `mapError()`. Il flusso auth web resta cookie-based con `api` (non usa il shared `auth.tsx`, che è desktop-first con sidecar+cloud). (PR #761)**                                                                                                                              |
