...
- [ ] **Admin license tier restrictions** — Il superadmin può assegnare/rimuovere solo il tier "lifetime" (enterprise). Non può modificare tier "pro" o "premium" (pagati via Stripe). Aggiungere colonna `license_tier_source` per tracciare origine della licenza (admin vs stripe vs free). Piano: `.specs/plans/feature-admin-license-restrictions.md`.
