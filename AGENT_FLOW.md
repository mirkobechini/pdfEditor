...
- **🔥 100% coverage per ogni file modificato prima della PR.** Ogni file toccato da un issue deve mantenere o raggiungere il **100% di statements coverage** (o comunque non scendere sotto il livello pre-modifica). Se un file scende di coverage a causa delle modifiche, l'agente DEVE aggiungere test per riportarlo al 100% prima di creare la PR. Eccezione: file con dipendenze non testabili in jsdom (es. canvas, DnD, WebGL) — in tal caso documentare il motivo in KNOWN_ISSUES.md.
