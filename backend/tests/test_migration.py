...
class TestMigrationIntegrity:
    ...
    def test_downgrade_single_and_upgrade_again(self):
        ...
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmp:
            ...
            # Verify latest migration (add_share_links_table) exists
            engine = create_engine(f"sqlite:///{db_path}")
            inspector = inspect(engine)
            tables_before = inspector.get_table_names()
            assert "sync_status" in tables_before
            assert "users" in tables_before
            # Check share_links table exists (latest migration)
            assert "share_links" in tables_before, f"share_links table should exist before downgrade: {tables_before}"
            _cleanup_engine(engine)

            # Downgrade one step (remove share_links table)
            rc, out, err = _run_migration(db_path, "downgrade -1")
            assert rc == 0, f"alembic downgrade -1 failed:\n{err}\n{out}"

            # Verify share_links is gone, but sync_status and other tables still exist
            engine = create_engine(f"sqlite:///{db_path}")
            inspector = inspect(engine)
            tables_after = inspector.get_table_names()
            assert "bug_reports" in tables_after
            assert "sync_status" in tables_after, "sync_status should survive this downgrade"
            assert "share_links" not in tables_after, f"share_links table should be gone after downgrade: {tables_after}"
            _cleanup_engine(engine)
            ...
            # Verify share_links is restored
            engine = create_engine(f"sqlite:///{db_path}")
            inspector = inspect(engine)
            tables = inspector.get_table_names()
            assert "share_links" in tables, f"share_links table should be restored after upgrade: {tables}"
